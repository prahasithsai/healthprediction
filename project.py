...
# sns.pairplot(model_bhvf.iloc[:,:])
# find and remove correlated features
corr=abs(model_bhvf.iloc[:,:-1].corr())
...
correlation(model_bhvf.iloc[:,:-1],threshold) # No values are found above threshold limit
...
skf.get_n_splits(X_bhvf,Y_bhv)
# x is the feature set and y is the target
for train_index,test_index in skf.split(X_bhvf,Y_bhv):
    print("Train:",train_index,"Validation:",test_index)
    x_train1,x_test1 = X_bhvf.iloc[train_index],X_bhvf.iloc[test_index]
    y_train1,y_test1 = Y_bhv.iloc[train_index],Y_bhv.iloc[test_index]
...
x_train2, x_test2, y_train2, y_test2 = train_test_split(X_bhvf, Y_bhv, test_size = 0.2,random_state=42)
...
x_train3, x_test3, y_train3, y_test3 = train_test_split(X_bhvf,Y_bhv, test_size = 0.2,random_state=42)
X_emtf = model_bhvf.iloc[:,:-1]
Y_emt = pd.DataFrame(model_bhvf.iloc[:,-1])
...
for train_index,test_index in skf.split(X_bhvf,Y_bhv):
    print("Train:",train_index,"Validation:",test_index)
    x_train3,x_test3 = X_bhvf.iloc[train_index],X_bhvf.iloc[test_index]
    y_train3,y_test3 = Y_bhv.iloc[train_index],Y_bhv.iloc[test_index]
...
dat = {'Logistic Regression':0.61,'Decision Tree':0.64,'RandomForest':0.65}
...
dat1 = {'Logistic Regression':0.59,'Decision Tree':0.62,'RandomForest':0.61}
modelname = list(dat.keys())
...
# Saving the model
import pickle
model = random.fit(x_train2,y_train2)
...
# model_bhvf['Are you part of school community'].value_counts()  
